...
class JaxKOutOfN:
    ...
    def __init__(
        ...
        self.env_name = f"{self.k}-out-of-{self.n_components}"
        ...
        if env_config["identical"]:
            replacement_accuracies = [
                env_config["replacement_accuracy"]
            ] * self.n_components
        else:
            replacement_accuracies = env_config["replacement_accuracies"]

        # replacement transition model
        # describes transition for (imperfect) replacements
        replacement_transition_model = np.zeros(
            (self.n_components, self.n_damage_states, self.n_damage_states)
        )

        for c in range(self.n_components):

            r = replacement_accuracies[c]
            replacement_transition_model[c] = np.array(
        ...
        for c in range(self.n_components):
            ...
            self.transition_model[c, 1] = (
                replacement_transition_model[c] @ self.deterioration_table[c]
            )
    ...
    def _compute_state_rewards(self, damage_state: jnp.array) -> float:

        # count number of components in failed state
        num_failed = jnp.sum(damage_state == self.n_damage_states - 1)
        ...
        _penalty = self.failure_cost * is_failed
    ...
    def step_env(
        ...
        # split keys into keys for replacement accuracies,
        ...
        terminated = self.is_terminal(timestep)
        truncated = False
